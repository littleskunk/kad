...
RPC.prototype.send = function(contact, message, callback) {
  ...
  if (Message.isRequest(message)) {
    ...
    this._log.info('replying message to %s', message.id);
  }
...
RPC.prototype.receive = function(buffer) {
  ...
  function deserialize() {
    ...
    if (contact.hdKey && message.method !== 'PUBLISH') {
      self._log.info('received valid %s message from %j', message.method, contact);
    } else {
      self._log.info('droping message %s message from %j', message.method, contact);
      throw new Error('droping message');
    }
  }
